...
pub mod cmd {
  ...
  pub fn control_window(handle: tauri::AppHandle, win_type: String) {
    ...
    if let Some(app_dir) = handle.path_resolver().app_config_dir()
    {
      info!("app_dir: {}",app_dir.display())
    }
    tauri::async_runtime::spawn(async move {
      if handle.get_window("core").is_none() {
        WindowBuilder::new(
          &handle,
          "core",
        ...
        let main_win = handle.get_window("core").unwrap();
